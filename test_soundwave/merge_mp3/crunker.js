...
class Crunker {
	...
	mergeAudio(buffers, delayTime) {
		
		// 딜레이 시작
		var startOffset = 0;

		// 딜레이 끝
		var endOffset = 44100 * (delayTime / 1000);
		
		var count = 0;
		
		let output = this._context.createBuffer(1, 44100 * this._maxDuration(buffers), 44100);
		let newArrayBuffer = this._context.createBuffer(output.numberOfChannels, endOffset - startOffset, output.sampleRate);
		...
		buffers.map(buffer => {
			if(count == 0){
				...
				for (let w = buffer.getChannelData(0).length; w >= 0; w--) {
					// + 할 수록 늦게되고, - 할 수 록 빨리 시작 됨
					output.getChannelData(0)[ w + endOffset] += buffer.getChannelData(0)[w];
				}
			}
			count ++;
		});
